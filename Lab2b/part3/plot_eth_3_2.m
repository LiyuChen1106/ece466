...
[packet_no_p, time_p, packetsize_p] = textread('generator_video_3_2.txt', '%f %f %f');
figure(1);
i=1;
time1(i)=0;
...
[time_p2, packetsize_p2, bufferSize, noToken] = textread('bucket_video_3_2.txt', '%f %f %f %f');
i=1;
time2(i)=0;
...
[packet_no_p3, time_p3, packetsize_p3] = textread('sink_video_3_2.txt', '%f %f %f');
i=1;
time3(i)=0;
...
plot(time3,arrival3+3e6,'r',time2,arrival2,'g',time3,arrival3,'b');
title('Cumulative arrival function of the generator, token bucket and sink for Video');
xlabel('time (in microseconds)');
ylabel('Cumulative arrivals (in bytes)');
%xlim([0,2.1e8]);
%legend 
