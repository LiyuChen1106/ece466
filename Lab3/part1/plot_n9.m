...
[time_p2, packetsize_p2, bufferSize, bufferSize0] = textread('ps_n9.txt', '%f %f %f %f');
...
fig1 = figure(1);
plot(time2,buffer,'-b');
title('Backlog in the buffer (N=9)');
...
[packet_no_p, time_p, packetsize_p] = textread('generator_n9.txt', '%f %f %f');
figure(1);
i=1;
time1(i)=0;
...
[packet_no_p3, time_p3, packetsize_p3] = textread('sink_n9.txt', '%f %f %f');
i=1;
time3(i)=0;
...
title('Number of Discarded packets (N=9)');
xlabel('Time (in microseconds)');
